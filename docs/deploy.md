...
- A fork version of this repository or a copy in a private repository.
...
- Check if
  ...
  command and go to the URL that is displayed.
...
Once the initial deployment only needs to be rub once using the `deploy.sh`
script. Updates should be made using the `apply.sh` script in the next section.
...
use the `apply.sh` script to make any updates.
